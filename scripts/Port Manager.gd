...
func load_input(port, key, value):
	...
	new_bar.get_child(1).get_child(0).selected = [1, 2, 4, 5, 6, 7].find(port)
...
func close_input(bar: Panel):
	Current_Inputs[[1, 2, 4, 5, 6, 7][bar.get_child(1).get_child(0).selected]].erase(bar.get_child(0).get_child(2).text)
	remove_child(bar)
	bar.queue_free()
...
func update_key(bar: Panel):
	...
	if bar.get_meta("PortChosen"):
		update_input(input_key, [1, 2, 4, 5, 6, 7][bar.get_child(1).get_child(0).selected], get_value(bar), bar)
...
func update_port(port: int, bar: Panel):
	...
	bar.set_meta("LastPort", [1, 2, 4, 5, 6, 7][port])
	if bar.get_meta("KeyChosen"):
		update_input(bar.get_child(0).get_child(2).text, [1, 2, 4, 5, 6, 7][port], get_value(bar), bar)

func update_value(bar: Panel):
	if bar.get_meta("KeyChosen") and bar.get_meta("PortChosen"):
		update_input(bar.get_child(0).get_child(2).text, [1, 2, 4, 5, 6, 7][bar.get_child(1).get_child(0).selected], get_value(bar), bar)
